...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        m = X.shape[1]
        self.coefs_ = np.zeros(m)
        for t in range(self.max_iter_):
            res = self.check_if_misclassified_exist(X, n, self.coefs_, y)
            if res == -1:
                return
            else:
                self.coefs_ += y[res] * X[res]
                self.callback_(self, X[res], y[res])

    @staticmethod
    def check_if_misclassified_exist(X, n, w, y) -> int:
        for i in range(n):
